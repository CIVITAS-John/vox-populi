...
bool CvImprovementEntry::CacheResults(Database::Results& kResults, CvDatabaseUtility& kUtility)
{
	...
	//YieldPerXAdjacentImprovement
	{
		...
		while (pResults->Step())
		{
			...
			PRECONDITION(improvement_idx > -1);
	...
	//AccomplishmentYieldChanges
	{
		...
		while (pResults->Step())
		{
			...
			PRECONDITION(accomplishment_idx > -1);
...
int CvImprovementEntry::GetAccomplishmentYieldChanges(int i, int j) const
{
	...
	PRECONDITION(j < NUM_YIELD_TYPES, "Index out of bounds");
	PRECONDITION(j > -1, "Index out of bounds");
	if (i < 0 || i >= NUM_ACCOMPLISHMENTS_TYPES - 1) return 0;
