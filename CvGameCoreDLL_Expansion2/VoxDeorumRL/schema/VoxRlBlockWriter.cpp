...
// Finds generated metadata for a section writer request.
static const VoxRlSectionDefinition* VoxRlWriterSectionDefinition(u16 sectionKind)
{
...
// Counts all generated sections belonging to a block kind.
static u16 VoxRlWriterBlockSectionCount(u16 blockKind)
{
    u16 count = 0;
    for (u16 index = 0; index < kVoxRlSectionDefinitionCount; ++index)
    {
...
// Counts required generated sections belonging to a block kind.
static u16 VoxRlWriterRequiredSectionCount(u16 blockKind)
{
    u16 count = 0;
    for (u16 index = 0; index < kVoxRlSectionDefinitionCount; ++index)
    {
...
// Confirms that a block kind was emitted by the schema generator.
static bool VoxRlWriterKnownBlockKind(u16 blockKind)
{
...
// Returns whether a block kind must fit the decision pipe payload cap.
static bool VoxRlWriterIsPipeBlockKind(u16 blockKind)
{
    ...
    u8 flags)
{
    ...
    if (!VoxRlWriterKnownBlockKind(blockKind))
    {
        return Fail(VOX_RL_BLOCK_BAD_KIND);
    }
    const u16 maximumSectionCount = VoxRlWriterBlockSectionCount(blockKind);
    if (sectionCount < VoxRlWriterRequiredSectionCount(blockKind) || sectionCount > maximumSectionCount)
    {
...
bool VoxRlBlockWriter::BeginSection(u16 sectionKind, u32 count, u32 byteLength, void** sectionBytes)
{
    ...
    const VoxRlSectionDefinition* definition = VoxRlWriterSectionDefinition(sectionKind);
    if (definition == 0)
    {
    ...
    u32 sectionOffset = 0;
    u32 sectionEnd = 0;
    if (!VoxRlAlignU32(nextOffset_, sectionOffset) ||
        !VoxRlAddU32(sectionOffset, byteLength, sectionEnd) || sectionEnd > kVoxRlMaximumImageBytes)
    {
        return Fail(VOX_RL_BLOCK_SECTION_LENGTH);
    }
    u32 blockEnd = 0;
...
bool VoxRlBlockWriter::Finish(u32* byteLength)
{
    ...
    if (nextOffset_ > kVoxRlMaximumImageBytes)
    {
        return Fail(VOX_RL_BLOCK_BAD_IMAGE_LENGTH);
    }
    if (VoxRlWriterIsPipeBlockKind(blockKind_) && nextOffset_ > kVoxRlMaximumFramePayloadBytes)
    {
